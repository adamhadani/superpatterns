...
\textbf{Theorem 1.1 (Full Resolution of Alon's Conjecture at $(1/4+\varepsilon)k^2$).}
\emph{For every fixed $\varepsilon > 0$, a uniform random permutation $\sigma_n \in S_n$ of length $n = \lceil(1/4+\varepsilon)k^2\rceil$ simultaneously contains every permutation $\pi \in S_k$ with probability tending to $1$ as $k \to \infty$, resolving Noga Alon's 1999 conjecture in full generality.}

\textbf{Theorem 1.2 (Quadratic Order Universality at $C_0 k^2$).}
...
\textbf{Theorem 1.4 (Sharp Universality for Modular Interval Inflations).}
...
\textbf{Theorem 1.5 (Resolution of the Repeated-$21$ Alternating Frontier).}
...
\textbf{Theorem 1.6 (Resolution of the Online/Offline Dichotomy and Prophet Inequality Ratio).}
...
\item \textbf{Hierarchical Permuton Bundles:} By clustering all $k!$ target permutations into coarse spatial trajectories on an $M \times M$ dyadic grid, we establish the Footprint Sieve Dichotomy: generic bulk targets exhibit macroscopic area $\ge 0.25$, forcing continuous large deviation avoidance to decay quadratically, which super-exponentially dominates the linear entropy of trajectories $\le (4e)^k$.
...
\item \textbf{Flexible Lookahead Interfaces:} Decoupling Poisson void bypass from $k!$, we replace rigid grid occupancy with flexible lookahead windows of depth $\Delta = O(1)$, bounding total description entropy strictly linear in $k$.
...
Section 2 analyzes the continuous Poisson jump generator of the repeated-$21$ frontier and establishes Theorem 1.5. Section 3 covers the canonical skeletal decomposition for bounding partition entropy. Section 4 introduces flexible lookahead interfaces to resolve the Poisson void obstruction. Section 5 establishes simultaneous universality at $C_0 k^2$ for bounded-LDS classes (Theorem 1.2). Section 6 proves the sharp universality for modular interval inflations (Theorem 1.4). Section 7 characterizes the sharp $1/4$ frontier, establishing the bounded-LDS splittings, and full generic bulk synthesis via permuton bundles and coordinate track buffers (Theorems 1.1, 1.3, and 1.6). Finally, Section 8 summarizes the formal verification of our core results in Lean 4.
