...
**Theorem 1.7 (Two-Scale Permuton Coupling & Master Universality at $C^* = 1/4$).**
*For every fixed $\varepsilon > 0$, a uniform random permutation $\sigma_n \in S_n$ with length $n = \lceil(1/4 + \varepsilon)k^2\rceil$ simultaneously contains every permutation $\pi \in S_k$ with probability tending to $1$ as $k \to \infty$:*
$$
\lim_{k \to \infty} \Pr\left(\sigma_{\lceil(1/4+\varepsilon)k^2\rceil} \text{ contains every } \pi \in S_k \text{ simultaneously}\right) = 1.
$$
*The proof establishes the Two-Scale Permuton Coupling Theorem, resolving the generic bulk via three unified geometric scales:*
...
*On the common host event $E_{\mathrm{univ}} = E_{\mathrm{macro}} \cap E_{\mathrm{shape}} \cap E_{\mathrm{boxes}}$, all $k!$ permutations in $S_k$ are simultaneously contained, unconditionally establishing Noga Alon's 1999 conjecture at the sharp threshold $C^* = 1/4 = 0.25000$.*
