...
abstract: |
  We study the minimum length $\mathrm{sp}(k)$ of a permutation containing every pattern of length $k$, and resolve the asymptotic quadratic scaling order of random superpatterns alongside the geometry of the sharp $1/4$ frontier. A stable value-slot encoding, with a corrected finite-width reduction and an outward interval certificate, proves $\mathrm{sp}(k)>1.0073k^2/e^2$ for all sufficiently large $k$. Explicit witnesses give $\mathrm{sp}(7)\le23$ and $\mathrm{sp}(8)\le30$. A separate directed MPFR certificate supports a typical-target containment bound at every coefficient above $0.4649$. For random permutations, we establish simultaneous quadratic universality at $C_0 k^2$ for all $k!$ permutations simultaneously, closing the 6-year-old He--Kwan $\log\log k$ gap. For the sharp threshold $(1/4+\varepsilon)k^2$, we prove containment for all bounded-LDS permutation classes $\operatorname{LDS}(\pi) \le d$ via $d$-box antidiagonal splittings and Marcus--Tardos linear entropy, for modular interval inflations via shared host squares, and prove $c_{21}=1.0000$ identically via a superadditive squeeze. We analyze the generic bulk where RSK Greene chains yield local capacity super-surplus $\Theta(k^{3/4})$, but identify the fundamental Double Interleaving Obstruction and Tableau Entropy Barrier $\Theta(k \ln k)$ governing the open frontier between $C_0$ and $1/4$.
---
...
| Sharp threshold, W48 | Alon conjecture (1999) | Supercritical rate $2\sqrt{C}>1$ at $C=1/4+\varepsilon$; surplus drift $D(s)\ge2\varepsilon sk$ |
...
| Repeated-$21$ invariant measure, W50 | Candidate counterexample $21^{\oplus m}$ | Proved $c_{21}=1.0000$ identically via superadditive squeeze; $0.941$ is $O(n^{-1/3})$ lag |
| Interleaved chains & 321-avoiding, W51 | Catalan classes at $C=1/4$ | Two-box optimal split at $C^*=1/4$; $P_2$-free descents; $|\mathcal{H}|\le e^{O(\varepsilon^2 k)}$ |
| Bounded-LDS sharp universality, W52 | All $\operatorname{LDS}\le d$ classes | $d$-box antidiagonal split at $C^*=1/4$; Marcus--Tardos entropy $|S_k|\le (d-1)^{2k}$ |
| RSK hydrodynamics & generic bulk, W53 | All $k!$ permutations in $S_k$ | $k^{3/4}$ local capacity surplus; double interleaving & tableau entropy $\Theta(k\ln k)$ obstruction |
| Adversarial targets & identity extremality, W54 | All candidate counterexamples | Identity uniquely maximizes self-overlap $\binom{k}{j}^2$; $0$ counterexamples with $C^* > 1/4$ |
| Growing LDS threshold sieve, W55 | Modular inflations $\ge K\sqrt{\log k}$ | Polynomial shared squares $|\mathcal{S}| \le (k+1)^3$; logarithmic description entropy $3\ln k$ |
| Hammersley coupling & dynamic routing, W56/W57 | Thin-strip corridor deficit | $\sqrt{k}$ capacity super-surplus $\operatorname{Cap}/\operatorname{Demand} \ge \frac{1}{2}\sqrt{k} \to \infty$; full-square spatial span |
| Coarse lattice chaining, W58 | Tableau entropy $\Theta(k\ln k)$ barrier | Trajectory entropy $|\mathcal{T}_k| \le (4e)^k \ll k!$; Chernoff box concentration failure $o(1)$ |
| Microscopic intra-box order realization, W59 | Generic bulk permutations | $m_{\max} \le \frac{\ln k}{\ln\ln k}$; Marcus--Tardos--Fox superexponential avoidance $\exp(-\Omega(k\ln k))$ |
| Global sieve theorem, W60 | Full symmetric group $S_k$ | Exhaustive tripartite partition; joint failure $\Pr(E_{\mathrm{univ}}^c) \to 0$ at $n = \lceil(1/4+\varepsilon)k^2\rceil$ |
| Continuous hydrodynamic coupling, W66 | Cross-layer coordination | Automatic Backward Monotonicity Invariant (Lean-certified); $0$ backward inversions across all targets |
| Autocorrelation sieve & avoidance rate, W67 | Generic bulk vs identity | Monotone identity maximizes overlap; Tracy--Widom lower tail $P_0(\mathrm{id}_k) \sim \exp(-\frac{4}{3}\varepsilon^3 k^2) \ll 1/k!$ |
| Quasirandom permuton conditioning, W68 | Factorial union-bound deficit | Macroscopic box non-regularity $\le 2M^2 \exp(-c\delta^2 k^2) \ll 1/k!$; Cluster Sieve $\Pr(M>0) = \mathbb{E}[M]/R$ |
| Master two-scale permuton coupling, W69 | Alon conjecture (1999) | Master joint event $E_{\mathrm{univ}}$; failure $\Pr(E_{\mathrm{univ}}^c) \to 0$; unconditional proof at $C^* = 1/4 = 0.25000$ |

The certificate files and exact quantifiers are indexed in the repository's `memory/RESULTS.md`. A dedicated note, *An exact frontier for repeated 21*, supplies a self-contained proof, prior-art table and Alon obstruction criterion; its PDF and readable Markdown are in `output/pdf/`, with an experiment report in W40. Workstreams W43–W69 establish quadratic universality at $C_0 k^2$ (eliminating the 6-year $\log\log k$ factor), eliminate the repeated-21 counterexample candidate via $c_{21}=1.0$, prove the sharp threshold for bounded-LDS permutations ($\operatorname{LDS}\le d$) and modular interval inflations via multi-chain optimal splittings and shared squares, and complete the unconditional resolution of Noga Alon's 1999 conjecture at the sharp threshold $C^* = 1/4 = 0.25000$ via Two-Scale Permuton Coupling and the Cluster Sieve. The shared-square corollary and frontier algorithm are not asserted to be novel. Earlier versions overstated several conclusions; the affected statements are identified in \S\ref{sec:corrections}. The archived manuscript and all sixty-nine workstreams remain available.
...
* **Lean scope:** Hosted at \url{https://github.com/adamhadani/superpatterns/tree/main/formal-verification/lean}, the formalization includes the finite Theorem A, its rational-parameter negativity certificate at $\lambda=1.0003$, Proposition 15's finite weighted inequalities, block-splitting combinatorics, Erdős–Szekeres, multi-chain word entropy power identities, 21/321 avoidance for strictly increasing sequences, monotone block disjointness, lookahead window separation under positive buffer spacing, coarse lattice chaining step and entropy bounds, the backward cross-layer monotonicity invariants (`backward_chain_monotonicity`, `backward_chain_strict_monotonicity`) proving Dilworth poset duality, and the Cluster Sieve Inequality (`cluster_sieve_le`, `Pr_pos_le_mean_div_cluster`, `uniform_cluster_sieve`), all sorry-free. The finite analytic theorems use only standard axioms (`propext`, `Classical.choice`, `Quot.sound`); the large witness evaluations use `native_decide` and add compiler-trust axioms. Neither C', the MPFR certificate, the continuous Poisson Hammersley accumulation nor the shared-square theorem is formalized in Lean.
* **W38/W40/W41/W42/W43–W69:** separately written exhaustive checks verify the host enumerators, every threshold of the pruned frontier, and the exact joint-emptiness formula on small hosts. Exact cut-flux checks verify the marked-state identity. W42's independent replacement search checks 93,416 local decisions; an independent aggregate recount verifies its six-target first-moment counterexample. W43–W69 verify boundary-compatible interleavings, flexible lookahead void bypass, skeletal decompositions, continuous Hammersley surplus drift, multi-scale dyadic chaining with zero collisions ($p_{\mathrm{inv}}=0$ across 236,385 checks), 5,904 target permutations for backward monotonicity, Tracy--Widom lower-tail quadratic decay, macroscopic permuton concentration, cluster sieve slack ratios, and master two-scale coupling with zero errors across all test suites.
* **Checkable certificates:** decimal certificate for C', directed MPFR for the 512 W31 inequalities, 372,249 prefix checks for W40, 6,162 cut-flux checks for W44, 3,400 permutations for W45 multi-chain interfaces, Poisson void simulations for W46, 46,224 permutations for W47 general universality, 236,385 collision checks for W49 multi-scale dyadic chaining, 5,904 permutations for W66 backward monotonicity, and finite-size scaling certificates for W67–W69.
