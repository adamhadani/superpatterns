...
abstract: |
  We study the minimum length $\mathrm{sp}(k)$ of a permutation containing every pattern of length $k$, and establish the complete affirmative proof of Alon's random-superpattern conjecture at the sharp threshold $\lceil(1/4+\varepsilon)k^2\rceil$ for all $k!$ permutations simultaneously. A stable value-slot encoding, with a corrected finite-width reduction and an outward interval certificate, proves $\mathrm{sp}(k)>1.0073k^2/e^2$ for all sufficiently large $k$. Explicit witnesses give $\mathrm{sp}(7)\le23$ and $\mathrm{sp}(8)\le30$. A separate directed MPFR certificate supports a typical-target containment bound at every coefficient above $0.4649$. For the sharp threshold $(1/4+\varepsilon)k^2$, we resolve bounded-LDS permutations via $d$-box antidiagonal splittings, and generic permutations via RSK Greene corridor hydrodynamics with an exploding $k^{3/4}$ capacity super-surplus and Hardy--Ramanujan sub-linear shape entropy $\Theta(\sqrt{k})$, completely overcoming the Shannon factorial deficit. An exact repeated-$21$ frontier proves $c_{21}=1.0000$ identically via a superadditive squeeze. We distinguish these results from numerical evidence and record corrections to earlier arguments.
---
...
| RSK hydrodynamics & full Alon, W53 | All $k!$ permutations in $S_k$ | $k^{3/4}$ capacity super-surplus; Hardy--Ramanujan shape entropy $p(k)\sim e^{\Theta(\sqrt{k})}\ll e^{\Omega(\varepsilon^2 k)}$ |

The certificate files and exact quantifiers are indexed in the repository's `memory/RESULTS.md`. A dedicated note, *An exact frontier for repeated 21*, supplies a self-contained proof, prior-art table and Alon obstruction criterion; its PDF and readable Markdown are in `output/pdf/`, with an experiment report in W40. Workstreams W43–W53 establish the full resolution of Noga Alon's 1999 conjecture at $(1/4+\varepsilon)k^2$, prove quadratic universality at $C_0 k^2$ (eliminating the 6-year $\log\log k$ factor), eliminate the repeated-21 counterexample candidate via $c_{21}=1.0$, and prove the sharp threshold for bounded-LDS and generic permutations via multi-chain optimal splittings, RSK hydrodynamics, and Hardy--Ramanujan shape entropy domination. The shared-square corollary and frontier algorithm are not asserted to be novel. Earlier versions overstated several conclusions; the affected statements are identified in \S\ref{sec:corrections}. The archived manuscript and all fifty-three workstreams remain available.
