...
## 2026-09-10 — independent review; corrections to earlier verification labels
- See [REVIEW-2026-09-10.md](REVIEW-2026-09-10.md) for the audit, counterexamples, literature update, and ranked next steps. This entry supersedes conflicting confidence/status claims above; the paper and workstream files have not yet been corrected.
- Re-ran `python3 experiments/witnesses/check_witness.py --all`: both the length-23 k=7 and length-30 k=8 witnesses pass exhaustive containment checks; the length-24 k=7 witness also passes. Lean build was not rerun in this review.
- Independently checked counterexamples to W3's both-parity stability assertion and W34's inference of revisit separation from H_b. W37 also reverses an LIS probability comparison and upgrades an o(k²) error without justification. W36's stationary independent-strip functional is separate from the W34 application gap.
- Recomputed W7's erroneous class-summed diagnostic (extra multiplicity division); the tested corrected per-class lemma still has zero violations. Inspected W38 raw outputs: the canonical off-diagonal term grows over the available sizes, and its boundedness and the plain exponential asymptotic remain unproved. Numerical certificates and rounded constants need the scope corrections detailed in the review.
- The August 2026 Altschuler–Dubroff–Tikhomirov paper implies stronger individual-target and typical-target benchmarks than W19/W29. Recommended focus: certify corrected C′; repair the structured-grid application; develop common host certificates. The review derives a shared-square proposition for all inflations by monotone blocks of length at least K_ε sqrt(log k), using classical LIS lower tails; novelty not established.
