...
static int R,H,K,N,FREE,DIM,BEAM;
...
int main(int argc,char**argv){
    ...
    int BF=0; for(int a=6;a<argc;a++){ if(!strcmp(argv[a],"-bf"))BF=1; else if(!strcmp(argv[a],"-free"))FREE=1; else if(!strcmp(argv[a],"-beam"))BEAM=atoi(argv[++a]); else if(!strcmp(argv[a],"-tau"))eps=atof(argv[++a]); }
    ...
    for(int rep=0;rep<reps;rep++){
        ...
        for(int i=0;i<N;i++) strip[i]=(int)(((long)Y[i]*R)/N);
