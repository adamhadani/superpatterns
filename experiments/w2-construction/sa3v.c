...
int main(int argc,char**argv){ k=atoi(argv[1]); m=atoi(argv[2]); B=atoi(argv[3]); long iters=atol(argv[4]); rs=atoll(argv[5])*2654435761ULL+1; n=m*B;
  ...
  for(long it=0;it<iters && best<fact[k];it++){
    ...
    long s=score();
    if(s>=cur || exp((s-cur)/T)*4294967296.0 > (double)(rnd()&0xffffffff)){ cur=s; if(s>best){best=s; memcpy(bw,w,sizeof(w)); fprintf(stderr,"it %ld best %ld/%ld :",it,best,fact[k]); for(int i=0;i<n;i++) fprintf(stderr," %d",bw[i]); fprintf(stderr,"\n"); fflush(stderr);} }
