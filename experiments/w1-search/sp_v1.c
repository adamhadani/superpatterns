...
static void full_count(void){
    ...
    dfs_full(0,0,0,0);
    missing=0; singles=0;
    for(int i=0;i<fact[k];i++){ if(!cnt[i]) missing++; else if(cnt[i]==1) singles++; }
}
...
static void dfs_pair(int p0,int m,uint64_t mask,uint64_t mask2,int c1,int c2){
    if(m==k){
        if(c1!=c2){
            { uint32_t c=--cnt[c1]; if(c==0){missing++;singles--;} else if(c==1) singles++; }
            { uint32_t c=cnt[c2]++; if(c==0){missing--;singles++;} else if(c==1) singles--; }
        }
        return;
    }
...
int main(int argc,char**argv){
    ...
    for(long it=0; it<iters; it++){
        ...
        double old=missing+LAM*singles;
        gen_move(sym);
        apply_moves();
        double d=missing+LAM*singles-old;
        if(d>0 && urand()>=exp(-d/T)) { undo_moves(); }
        else if(it==0) { }
        ...
        if(since>RH){ T=T0; since=0; memcpy(sig,bestsig,sizeof(sig)); for(int i=0;i<n;i++) pos[sig[i]]=i; full_count(); fprintf(stderr,"reheat at it=%ld\n",it);} 
